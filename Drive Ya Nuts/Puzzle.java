...
public class Puzzle {
    ...
    // Given an array list of currently placed pieces, returns an arraylist of the pieces that could legally
    // be placed in the next available slot. Pieces are placed in the following order: the zeroth element is
    // the hub; the next piece is placed so that it lines up with the "1" of the hub pieces, and all following
    ...
    public ArrayList<Piece> findPotentialPieces(ArrayList<Piece> piecesSoFar) {
        ...
        default: // Finds pieces that match both the next open spot on the hub and the open side on
                 // the previously placed piece. If this is the seventh and final piece, the piece
                 // must also match the other open side on the first edge piece placed.
    ...
    // placed next, adds it to a new array list of pieces already placed, and recurses on the new arraylist.
